...
-- Insert sample bouquets (убрал ON CONFLICT для bouquets)
...
-- Create users with hashed passwords (используйте реальные BCrypt хеши)
...
-- Insert order items (убрал ON CONFLICT для order_items)
...
-- Insert sample reviews (убрал ON CONFLICT для reviews)
